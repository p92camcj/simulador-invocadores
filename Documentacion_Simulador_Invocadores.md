...
| `decidirYJugarTurno(players, neutrals, botIdx, contexto)` | `función` | Punto de entrada único: despacha por `players[botIdx].dificultad` (`HEURISTICAS_POR_DIFICULTAD`, hoy solo `'normal'`) y ejecuta el turno completo (Fases A-E) de la bot. `contexto = { levelIdx, invocationSet }`. Heurística `'normal'`: Fase A prioriza jugar la carta conocida en un Portal bloqueante (vacío/oculto) si eso completa la invocación activa; si no, prefiere la conocida (para no arriesgar duplicar sin querer un personaje necesario) en un Portal razonable (bloqueante, o que no tape un personaje del combo); solo jugaría la oculta propia si no hubiera conocida (no ocurre en la práctica). Fase B activa como mucho Ocultista o Cronista, solo si falta algún personaje del combo por revelar y hay un Portal oculto legal donde intentarlo — Estratega/Cronomante/Aprendiz/Metamorfo quedan fuera del MVP. Los picker()/pickerPortal() que abre `applyAbility()` se resuelven programáticamente (mismo `#pickerSelect`/`#pickerOk` que usaría un clic humano). Termina simulando el clic en `#btnEndTurn`, que ya comprueba invocación, reparte Gemas, roba y avanza turno con la misma lógica que una jugadora humana. Ver `docs/MEJORAS_FUTURAS.md`, "Jugador autómata: niveles de dificultad", para el punto de extensión de niveles futuros y de memoria entre turnos. |
