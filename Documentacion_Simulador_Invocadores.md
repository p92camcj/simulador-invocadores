...
| `decidirYJugarTurno(players, neutrals, botIdx, contexto)` | `función` | Punto de entrada único: despacha por `players[botIdx].dificultad` (`HEURISTICAS_POR_DIFICULTAD`: `'normal'` o `'dificil'`) y ejecuta el turno completo (Fases A-E) de la bot. `contexto = { levelIdx, invocationSet }`. Fase B activa como mucho Ocultista o Cronista (heurística `'normal'`) — solo si falta algún personaje del combo por revelar y hay un Portal oculto legal donde intentarlo; Estratega/Cronomante/Aprendiz/Metamorfo/Maestro quedan fuera de este nivel. Heurística `'dificil'` (ver `js/bot-probabilidad.js`): Fase B decide por valor esperado en Gemas (conteo de cartas + memoria propia del bot, `window.memoriaBots[botIdx]`) y también sabe usar Maestro (determinista, la carta objetivo ya es una identidad conocida con certeza). Ambos niveles resuelven los `picker()`/`pickerPortal()` que abre `applyAbility()` programáticamente (mismo `#pickerSelect`/`#pickerOk` que usaría un clic humano; para `'dificil'`, el Portal/jugadora concreto ya calculado se pasa como `objetivoPreferido` al primer picker que se abra) y el resumen de turno (`alert()`) siempre se redacta en tercera persona, incluyendo de quién es el Portal/mano afectado por la habilidad activada (`describirObjetivoHabilidad`, arriba). Termina simulando el clic en `#btnEndTurn`, que ya comprueba invocación, reparte Gemas, roba y avanza turno con la misma lógica que una jugadora humana. |
