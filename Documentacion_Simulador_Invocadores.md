...
| `render(players, neutrals, levelIdx)` | `función` | Dibuja: <br>- Zona activa (jugadora actual, portales, mano, suma de Gemas vía `sumaGemas()`). Cada carta de la mano lleva `draggable="true"` y `onclick="window.selectHandCard(idx)"`/`ondragstart="window.handleCardDragStart(...)"` (definidos en `actions.js`); recibe la clase `.selected` si es `window.selectedCardIdx`. Cada Portal (propio, ajeno o central, en cualquiera de las tres zonas) lleva `onclick="window.tryPlayOnPortal(destKey)"` + `ondragover`/`ondrop` (helper interno `portalPlayAttrs(destKey)`), y la clase `.target-portal` mientras haya una carta seleccionada. <br>- Zona de otros jugadores (portales + sus cartas ocultas; si `p.hasClariActivo \|\| p.haTenidoClarividente`, las DOS cartas de esa jugadora se muestran como "Carta oculta" al resto, sobrescribiendo la visibilidad normal por carta — ver nota de Clarividente en `docs/reglamento/REGLAMENTO.md`). <br>- Portales neutrales. <br>- Estado y componentes de la invocación activa, resuelta desde `INVOCATION_SETS[window.invocationSet][lvl]` (nombre y `need`). <br>- Muestra/oculta `#btnPlayCancel` según haya o no una carta seleccionada. <br>- Si `window.debugViewActive` es `true`, sustituye por completo la vista compartida normal (`#sectionflex`, `#zoneNeutral`) por `renderDebugGrid()` en `#debugView`, en vez de mostrarlas (esta vista de depuración NO tiene selección/clic para jugar cartas — es solo de inspección). <br>Mano y Portales (propios, ajenos y centrales) ya no muestran emoji/texto: usan la función interna `cartaImgHtml(name, visible)`, que renderiza un `<img>` con `cardImages[name]` si la carta es visible para quien mira, o siempre `CARTA_OCULTA_IMG` (con `alt="Carta oculta"`, sin el nombre real) si no lo es. |
| `renderDebugGrid(players, neutrals)` | `función` | Vista de depuración SOLO para pruebas del dueño del proyecto (no usar en partida real): un grid CSS con una columna por jugadora (`grid-template-columns: repeat(N, 1fr)`, N = nº de jugadoras + 1 si hay Portales centrales) mostrando SIEMPRE el nombre real de cada carta (`cartaImgHtml(name, true)`), ignorando `vis.public`/`owner`/`others` por completo. Cada columna: Portales de esa jugadora arriba, mano en un sub-grid de 2 columnas debajo. Los Portales centrales van en una columna final "Neutrales". Se invoca automáticamente desde `render()` cuando `window.debugViewActive` es `true` — no hace falta llamarla aparte. |
...
| Nombre              | Tipo      | Descripción                                                                                                                                        |
|---------------------|-----------|----------------------------------------------------------------------------------------------------------------------------------------------------|
| `initActions(players, neutrals)` | `función` | Jugar carta (Fase A) es ahora selección directa por clic o drag&drop, no un panel con `<select>`: <br>- `window.selectHandCard(idx)`: clic en una carta de la propia mano; toggle (un segundo clic sobre la misma la deselecciona). No hace nada si ya se jugó carta este turno. Guarda el índice en `window.selectedCardIdx` (reseteado en `nextTurn()`/`resetJuego()`/`initGame()`) y llama a `render()`. <br>- `window.tryPlayOnPortal(destKey)`: clic en un Portal (propio, central o de otra jugadora; `destKey` = `'p:<idx>'` \| `'n:<idx>'` \| `'a:<pi>:<pj>'`); si hay una carta seleccionada la juega ahí (mismo efecto que antes: `stack.push(...)`, hook de `ocultarOtrasCentinelas()` si es Centinela, `window.played = true`), si no, no hace nada. <br>- `window.handleCardDragStart`/`handlePortalDragOver`/`handlePortalDrop`: drag&drop nativo como alternativa que coexiste con el clic — mismo efecto final que `tryPlayOnPortal`. <br>- `#btnPlayCancel.onclick`: limpia `window.selectedCardIdx` y refresca (el propio `render()` decide si el botón está visible, según si hay selección activa). <br>- `#btnAbility.onclick`: Fase B, una vez por turno. Si hay una investigación de Cronomante pendiente (`window.cronomantePortalInvestigado` con valor), SALTA el picker de nivel superior y llama directamente a `applyAbility('Cronomante', ...)` con el `onComplete` guardado en `window.cronomanteOnComplete` — no se puede elegir otra habilidad hasta completar o que cambie el turno. Si no, construye opciones con `opcionesActivarHabilidad()`; al elegir una, cobra el coste con `pagarActivacionPortalCentral()` si la fuente es un Portal central, marca `window.habilidadUsadaEsteTurno` y llama a `applyAbility()` — si la habilidad es Cronomante, guarda ese `onComplete` en `window.cronomanteOnComplete` antes de llamar, por si hace falta reintentar tras un cancel (ver `abilities.js`). <br>- `#btnEndTurn.onclick`: valida jugada, reparte robo si <2 cartas, comprueba invocación contra `INVOCATION_SETS[window.invocationSet][lvl]`, reparte Gemas reales (`construirPoolGemas` + bonus de Pícaro/Maestro), añade portal neutral, avanza turno y refresca. <br>- `#btnDebugView.onclick`: alterna `window.debugViewActive` y llama a `render()` — es `render()` quien decide, según ese flag, si muestra la vista compartida normal o `renderDebugGrid()` (ver `render.js`). |
