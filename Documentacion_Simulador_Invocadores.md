...
| `decidirYJugarTurno(players, neutrals, botIdx, contexto)` | `función` | Punto de entrada único: despacha por `players[botIdx].dificultad` (`HEURISTICAS_POR_DIFICULTAD`: `'normal'` o `'dificil'`) y ejecuta el turno completo (Fases A-E) de la bot. `contexto = { levelIdx, invocationSet }`. Heurística `'normal'`: Fase A prioriza jugar la carta conocida en un Portal bloqueante (vacío/oculto) si eso completa la invocación activa; si no, prefiere la conocida (para no arriesgar duplicar sin querer un personaje necesario) en un Portal razonable (bloqueante, o que no tape un personaje del combo); solo jugaría la oculta propia si no hubiera conocida (no ocurre en la práctica). Fase B activa como mucho Ocultista o Cronista, solo si falta algún personaje del combo por revelar y hay un Portal oculto legal donde intentarlo — Estratega/Cronomante/Aprendiz/Metamorfo/Maestro quedan fuera de este nivel. Heurística `'dificil'` (ver `js/bot-probabilidad.js`): Fase A y Fase B deciden por valor esperado en Gemas (conteo de cartas + memoria propia del bot, `window.memoriaBots[botIdx]`) en vez del atajo greedy — Fase B también sabe usar Maestro (determinista, la carta objetivo ya es una identidad conocida con certeza). Ambos niveles resuelven los `picker()`/`pickerPortal()` que abre `applyAbility()` programáticamente (mismo `#pickerSelect`/`#pickerOk` que usaría un clic humano; para `'dificil'`, el Portal/jugadora concreto ya calculado se pasa como `objetivoPreferido` al primer picker que se abra). Termina simulando el clic en `#btnEndTurn`, que ya comprueba invocación, reparte Gemas, roba y avanza turno con la misma lógica que una jugadora humana. |
